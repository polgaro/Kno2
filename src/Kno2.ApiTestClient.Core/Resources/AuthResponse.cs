    ...
    public class AuthResponse : IToken
    {
        ...
        public TokenRequestType TokenRequestType
        {
            get
            {
                ...
                {
                    string.Format(" √ No access token has been created / saved").ToConsole();
                ...
                {
                    string.Format(" √ Access token has expired: Expires {0} < Now {1}", utcNow, Expires).ToConsole();
                    ...
                    {
                        string.Format(" √ Refresh token has also expired: Expires {0} < Now {1}", utcNow, RefreshTokenExpires).ToConsole();
                        return TokenRequestType.ClientCredential;
                    }

                    string.Format(" √ Refresh token is still valid for {0} seconds", (RefreshTokenExpires - utcNow).TotalSeconds).ToConsole();
                    return TokenRequestType.RefreshToken;
                }

                // access token is still in the window so use the current access token
                string.Format(" √ Access token is still valid for {0} seconds", (Expires - utcNow).TotalSeconds).ToConsole();
